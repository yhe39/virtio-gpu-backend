 ...
 * Host capabilities
 */
#define VIRTIO_GPU_S_HOSTCAPS	(1ULL << VIRTIO_F_VERSION_1) | \
				(1ULL << VIRTIO_GPU_F_EDID) | \
				(1ULL << VIRTIO_GPU_F_MODIFIER)
...
virtio_gpu_cmd_resource_create_2d(struct virtio_gpu_command *cmd)
{
	...
	pr_dbg("%s: resource_id=%d format=0x%x width=%d height=%d \n",
			__func__, req.resource_id, req.format, req.width, req.height);
...
virtio_gpu_cmd_set_scanout(struct virtio_gpu_command *cmd)
{
	...
	if ((req.r.x > r2d->width) ||
		...
		pr_dbg("%s: x/y/w/h=%d/%d/%d/%d stride=%d bytes_pp=%d format=0x%x pixel=0x%x\n",
				__func__, req.r.x, req.r.y,req.r.width,req.r.height, surf.stride, bytes_pp,
				r2d->format, surf.pixel);
...
static void
virtio_gpu_notify_controlq(void *vdev, struct virtio_vq_info *vq __attribute__((unused)))
{
	struct virtio_gpu *gpu;
...
static void
virtio_gpu_notify_cursorq(void *vdev, struct virtio_vq_info *vq __attribute__((unused)))
{
	struct virtio_gpu *gpu;
...
static int
virtio_gpu_init(struct vmctx *ctx, struct pci_vdev *dev, char *opts __attribute__((unused)))
{
	struct virtio_gpu *gpu;
	...
	if (vm_allow_dmabuf(gpu->base.dev->vmctx)) {
		...
		if (gpu->is_blob_supported)
			gpu->base.device_caps |= (1ULL << VIRTIO_GPU_F_RESOURCE_BLOB);
	}
	...
	pr_dbg("%s vga w/h=%d/%d fb_base=0x%x\n", __func__, info.width, info.height, ctx->fb_base);
...
static void
virtio_gpu_deinit(struct vmctx *ctx __attribute__((unused)), struct pci_vdev *dev, char *opts __attribute__((unused)))
{
	struct virtio_gpu *gpu;
	struct virtio_gpu_resource_2d *r2d;
	int i;
...
uint64_t
virtio_gpu_edid_read(struct vmctx *ctx __attribute__((unused)), int vcpu __attribute__((unused)), struct pci_vdev *dev,
